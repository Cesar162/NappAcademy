...
class Pedido:
    ...
    def checkout(self, forma_pagamento = None):
        if forma_pagamento is None: 
            raise Exception
        if forma_pagamento.lower() in self.__class__.forma_aceitas:
            dados_checkout = (self.nota_fiscal(), self.valor_total_pagamento())
            return dados_checkout
        raise Exception ('Informe o meio de pagamento')
